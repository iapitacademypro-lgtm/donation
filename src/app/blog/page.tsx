...
export default async function BlogsPage() {
  ...
  posts = await getAllPosts().catch((error) => {
    console.error("Failed to load blog posts:", error)
    errorMessage = "Unable to load blog posts at this time."
    return []
  })
    ...
    >
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="mb-4 sm:mb-6">
          <Link href="/support" className="inline-flex items-center gap-2 text-black hover:text-cyan-800 w-fit p-2">
        ...
        {/* Cards */}
        {errorMessage ? (
          <div className="rounded-3xl border border-black/10 bg-white/80 p-8 text-center text-sm sm:text-base text-black/80">
            {errorMessage}
          </div>
        ) : posts.length === 0 ? (
          <div className="rounded-3xl border border-black/10 bg-white/80 p-8 text-center text-sm sm:text-base text-black/80">
